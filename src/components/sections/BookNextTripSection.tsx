    ...
    <section className='flex justify-between flex-col md:flex-row items-center mt-[-10rem]'>
      <div>
        ...
        <div className='flex flex-col gap-[3rem] mt-[1.94rem]'>
          {steps.map((step) => (
            <TripStepCard key={step.id} title={step.title} description={step.description} iconUrl={step.iconUrl} />
          ))}
        </div>
      </div>
      <div>
        <Image src='/images/next-trip.png' alt='card with a girl on a wall' width={655} height={789} />
      </div>
    </section>
  )
}
